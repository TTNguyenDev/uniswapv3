...
contract UniswapV3NFTManager is ERC721 {
    ...
    ) ERC721("UniswapV3 NFT Position", "UNIV3") {
    ...
    {
        TokenPosition memory tokenPosition = positions[params.tokensId];
        if (tokenPosition.pool == address(0x00)) revert WrongToken();
    ...
    function burn(uint256 tokenId) public isApprovedOrOwner(tokenId) {
        ...
        IUniswapV3Pool pool = IUniswapV3Pool(tokenPosition.pool);
        (uint128 liquidity, , , uint128 tokensOwed0, uint128 tokenOwed1) = pool
            .positions(poolPositionKey(tokenPosition));

        if (liquidity > 0 || tokensOwed0 > 0 || tokenOwed1 > 0)
